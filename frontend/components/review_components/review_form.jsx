...
class ReviewForm extends React.Component {
  ...
  handlePostReview(e) {
    ...
    this.props.createReview(tempReview).then(() => {
  ...
  render() {
    ...
    if (this.props.reviewErr) {
      errors = this.props.reviewErr.map((err, i) => {
        return (
          <div className='review-error' key={`err-${i}`}>
            {err}
          </div>
        );
      });
    ...
    const requestLogin = (
      <div>
        Please <Link to='/login'>login</Link> to post/edit review.
      </div>
    );

    if (this.state.review_posted) {
      return (
        <div className='biz-review-form'>Your review has been posted.</div>
      );
    } else {
      return (
        <div>
