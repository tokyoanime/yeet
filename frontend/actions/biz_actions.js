...
export const getBiz = (id) => (dispatch) => {
  return BizUtil.getBiz(id).then(
    (biz) => dispatch(receiveBiz(biz)),
    (err) => dispatch(receiveBizErrors(err))
  );
};

export const updateBiz = (biz) => (dispatch) =>
  BizUtil.updateBiz(biz).then(
    (biz) => dispatch(receiveBiz(biz)),
    (err) => dispatch(receiveBizErrors(err))
  );

export const searchBiz = (query) => (dispatch) =>
  BizUtil.searchBiz(query).then(
    (biz) => dispatch(receiveAllBiz(biz)),
    (err) => dispatch(receiveBizErrors(err))
  );

export const liveSearch = (query) => (dispatch) =>
  BizUtil.liveSearch(query).then(
    (biz) => dispatch(searchResults(biz)),
    (err) => dispatch(receiveBizErrors(err))
  );
