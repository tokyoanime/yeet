class Api::BusinessesController < ApplicationController
  ...
  def search
    ...
    keyword = query["keyword"].downcase
    near = query["near"].downcase
    
    @biz = Business.where('lower(biz_name) LIKE ? OR lower(biz_first_cat) LIKE ? OR lower(biz_second_cat) LIKE ? OR lower(biz_third_cat) LIKE ?', "%#{keyword}%", "%#{keyword}%", "%#{keyword}%", "%#{keyword}%").where('lower(biz_city) LIKE ?', "%#{near}%")
    render :index
  end
